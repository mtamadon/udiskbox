...
void Usart_Configuration(USART_TypeDef * USART_X, uint32_t BaudRate)
{
	...
	USART_InitStructure.BaudRate            = BaudRate ;	  			//波特率115200
	USART_InitStructure.WordLength          = USART_WordLength_8b; 	//传输过程中使用8位数据
	USART_InitStructure.StopBits            = USART_StopBits_1;	 	//在帧结尾传输1位停止位
	USART_InitStructure.Parity              = USART_Parity_No ;	 	//奇偶失能
	USART_InitStructure.HardwareFlowControl = USART_HardwareFlowControl_None;//硬件流失能
	USART_InitStructure.Mode                = USART_Mode_Rx | USART_Mode_Tx; //接收和发送模式
